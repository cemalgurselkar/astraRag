...
class HybridRetriever:
    ...
    def retriever(
        ...
        dense_results = self.dense_retri.retriever(query=query, top_k=candidate_k)

        sparse_results = self.sparse_retri.retriever(query=query, top_k=candidate_k)
        ...
        return [
            results_by_id[chunk_id].model_copy(update={"score": fused_scores[chunk_id]})
